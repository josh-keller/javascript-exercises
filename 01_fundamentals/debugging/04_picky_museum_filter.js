function wantToVisit(museum, city) {
  const isCorSMuseum = museum.includes('Computer') || museum.includes('Science');
  const isModernArtMuseum = museum.includes('Modern') && museum.includes('Art');
  const isAndyWarholMuseum = museum.includes('Andy') && museum.includes('Warhol');

  return isCorSMuseum
         || (isModernArtMuseum
            && (isAndyWarholMuseum || city === 'Amsterdam'));
}
